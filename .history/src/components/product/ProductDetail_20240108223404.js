    ...
    const [product, setProduct] = useState(null);
      ...
      // }
        const ProductDetailId = async (id) => {
            try {
                const res = await productDetail(id, cateId);
                setProduct(res.data);
                console.log(res);
            } catch (error) {
                console.log(error);
          ...
          {cateId === 1 && (
            <div>
              ...
              <div>
                ...
                <p>{product.capsuleP.output}</p>
                <p>{product.capsuleP.capsuleSize}</p>
                <p>{product.capsuleP.machineDimension}</p>
                <p>{product.capsuleP.shippingWeight}</p>
              </div>
            </div>
          )}
          {cateId === 2 && (
            <div>
              {/* Nội dung cho cateId là 2 */}
            </div>
          )}
          {cateId === 3 && (
            <div>
              {/* Nội dung cho cateId là 3 */}
            </div>
          )}
        </div>
      )}
