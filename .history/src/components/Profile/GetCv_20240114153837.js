    ...
    const handleSubmit = async (orderId) => {
        try {
            const rs = await axios.get(
                `${process.env.REACT_APP_API_URL}/Profile/candidate/delete?idCandidate=${idCandidate.id}&idProfileDetail=${orderId}`,
                {
                    headers: {
...
//xac nhan cv 
    const handleOrderDetail = async (orderId) => {
        try {
            const rs =  await axios.post (`${process.env.REACT_APP_API_URL}/Profile/candidate/submit?idCandidate=${idCandidate.id}&idProfileDetail=${orderId}`,{ },
            {
                headers: {
                    "Content-Type": "application/json",
...
console.log("Profile",idCandidate.id);
    ...
    useEffect(() => {
        const getProfile = async () => {
            try {
                const response = await axios.get(`${process.env.REACT_APP_API_URL}/Profile/candidate/getprofile?idCandidate=${idCandidate.id}&status=EDIT`, {
                    headers: {
                        "Content-Type": "application/json",
                        Authorization: `Bearer ${accessToken}`,
                    },
...
///get ra lich su
    const getProfileHistory = async () =>{
      try{
        const res = await axios.get(`${process.env.REACT_APP_API_URL}/Profile/candidate/gethistory?idCandidate=${idCandidate.id}`,{
          headers : {
            "Content-Type" : "application/json",
            Authorization: `Bearer ${accessToken}`,
          }
...
const getProfileDetails = async (orderId) => {
 try{
  const res = await axios.get(`${process.env.REACT_APP_API_URL}/Profile/admin/getdetail?IdProfile=${orderId}`,{
              ...
              <div class="site-navigation">
                <ul class="">
                  ...
                  <li class="nav-item pl-3">
                    <NavLink  class="nav-link px-2" onClick={getProfile}>
                          ...
                          {data.length >= 0 &&
                            ...
                            <tr>
                              <td class="text-nowrap align-middle">{data.id}</td>
                                            ...
                                            <button className="bg-primary border-0 text-white"  data-toggle="modal" data-target="#user-form-modal " type="submit" onClick={() => getProfileDetails(data.id) }>Details</button>
                                              </td> 
                              <td class="text-center align-middle">
                                <div class="btn-group align-top">
                                    ...
                                    <button className="bg-danger pl-2 border-0 mr-3 text-white" type="submit" onClick={() => handleSubmit(data.id) }> Huỷ</button>
