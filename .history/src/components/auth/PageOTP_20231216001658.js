...
const PageOTP = () => {
  ...
  useEffect(() => {
    ...
    if(waitingTime <= 30 && waitingTime >=0) {
     setInterval(() => {
      intervalId = setWaitingTime((prevWaitingTime) => prevWaitingTime - 1);
      },1000)
    }
    else {
      setSentBack(true);
  ...
  }, [waitingTime]); // Thực hiện một lần khi component được tạo
  //gửi otp
        ...
        <form onSubmit={handleLogin}>
          ...
          {!sentBack ? (
                    <div className="flex flex-col gap-1 w-full h-full ">
                        ...
                        <p className="text-sm  text-secondary ">Mã xác minh có hiệu lực trong 3 phút</p>
                    </div>
                ) : (
                    <div className="flex flex-col gap-1 w-full h-full ">
                        <p className="flex gap-2 items-center text-secondary text-sm">
                            Không nhận được?
                            <span className="text-sm text-primary cursor-pointer" onClick={handleSendOtp}>
                                Gửi lại mã
                            </span>
                        </p>
                    </div>
